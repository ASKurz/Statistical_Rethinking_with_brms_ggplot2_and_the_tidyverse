...
That `map_dbl()` code within the first `mutate()` line might look odd. Go [here](https://purrr.tidyverse.org/reference/rerun.html) to learn more about iterating with `purrr::map_dbl()`.
...
tibble(mu    = 0,
  ...
  geom_line() +
  ylab(expression(paste(italic(p), "(", italic("y|"), mu==0, ",",  sigma==1, ")")))
```
...
Let's get the data from McElreath's [rethinking package](https://xcelab.net/rm/statistical-rethinking/).
...
```{r b4.1, echo = F}
# save(b4.1, file = "fits/b04.01.rda")
# rm(b4.1)
load("fits/b04.01.rda")
```
...
McElreath's uniform prior for $\sigma$ was rough on brms. It took an unusually-large number of warmup iterations before the chains sampled properly. As McElreath covered in Chapter 8, Hamiltonian Monte Carlo (HMC) tends to work better when you default to a half Cauchy for $\sigma$. We can do that like this.

```{r b4.1_hc, echo = F}
# save(b4.1_hc, file = "fits/b04.01_hc.rda")
# rm(b4.1_hc)
load("fits/b04.01_hc.rda")
```
...
```{r b4.2, echo = F}
# save(b4.2, file = "fits/b04.02.rda")
# rm(b4.2)
load("fits/b04.02.rda")
```
...
I had to increase the `warmup` due to convergence issues. After doing so, everything looks to be on the up and up. The chains look great. And again, we will learn more about these technical details in Chapter 8.
...
This only returned the first element in the matrix it did for rethinking. That is, it appears `brms::vcov()` only returns the variance/covariance matrix for the single-level $\beta$ parameters (i.e., those used to model $\mu$).

However, if you really wanted this information, you could get it after putting the HMC chains in a data frame. We do that with the `posterior_samples()`, which we'll be using a lot as we go along.
...
```{r b4.3, echo = F}
# save(b4.3, file = "fits/b04.03.rda")
# rm(b4.3)
load("fits/b04.03.rda")
```
...
```{r b4.4, echo = F}
# save(b4.4, file = "fits/b04.04.rda")
# rm(b4.4)
load("fits/b04.04.rda")
```
...
```{r b4.3_010, echo = F}
# save(b4.3_010, file = "fits/b04.03_010.rda")
# save(b4.3_050, file = "fits/b04.03_050.rda")
# save(b4.3_150, file = "fits/b04.03_150.rda")
# save(b4.3_352, file = "fits/b04.03_352.rda")
# rm(b4.3_010)
# rm(b4.3_050)
# rm(b4.3_150)
# rm(b4.3_352)

load("fits/b04.03_010.rda")
load("fits/b04.03_050.rda")
load("fits/b04.03_150.rda")
load("fits/b04.03_352.rda")
```
...
If you wanted to express those sweet 95% HPDIs on your density plot, you might use the `tidybayes::stat_pointintervalh()` function. Since `stat_pointintervalh()` also returns a point estimate, we'll just throw in the mode.
...
```{r b4.5, echo = F}
# save(b4.5, file = "fits/b04.05.rda")
# rm(b4.5)
load("fits/b04.05.rda")
```
...
```{r b4.6, echo = F}
# save(b4.6, file = "fits/b04.06.rda")
# rm(b4.6)
load("fits/b04.06.rda")
```
...
```{r b4.7, echo = F}
# save(b4.7, file = "fits/b04.07.rda")
# rm(b4.7)
load("fits/b04.07.rda")
```
