...
f <- rbind(fitted(b7.1, newdata = nd),
...
4. "Once you begin using multilevel models (@sec-Multilevel-Models), you'll see that there are advantages to borrowing information across categories like 'Africa' and 'not Africa'" (p. 214).
...
Using the skills from @sec-Comparing-WAIC-values, let's compute the information criteria for the two models. Note how with the `add_criterion()` function, you can compute both the LOO and the WAIC at once.
...
\begin{align*}
\text{log\_gdp}_i & \sim \operatorname{Normal}(\mu_i, \sigma) \\
...
$$\mu_i = \alpha + (\beta_1 + \beta_3 \text{cont\_africa}_i) \text{rugged}_i + \beta_2 \text{cont\_africa}_i$$
...
```{r, message = F}
b7.5 <- add_criterion(b7.5, criterion = c("loo", "waic"))
...
```{r, message = F}
b7.5b <- add_criterion(b7.5b, criterion = c("loo", "waic"))
...
```{r}
f <- fitted(b7.5, newdata = nd) %>%  # we can use the same `nd` data from last time
  data.frame() %>%
  bind_cols(nd) %>%
  mutate(cont_africa = ifelse(cont_africa == 1, "Africa", "not Africa"))
```
...
And now we'll factor together the terms containing $\text{cont\_africa}_i$:
...
```{r}
b7.6 <- add_criterion(b7.6, criterion = "waic")
b7.7 <- add_criterion(b7.7, criterion = "waic")
...
$$\mu_i \mid \text{shade}_{i = 2}, \text{water}_{i = 2} = \alpha + \beta_\text{water} (2) + \beta_\text{shade} (2) + \beta_{\text{water} \times \text{shade}} (2 \times 2)$$
...
$$\mu_i \mid \text{shade}_{i = 2}, \text{water}_{i = 2} = -106.2 + 159.3 (2) + 43.4 (2) -42.9 (2 \times 2)$$
...
```{r, fig.height = 3, fig.width = 3.5, message = F}
