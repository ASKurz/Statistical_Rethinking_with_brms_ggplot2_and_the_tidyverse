...
We'll name our custom theme `theme_pearl_earring()`. I cobbled together this approach to defining a custom ggplot2 theme with help from [Chapter 16 of Wichkam's *ggplot2: Elegant Graphics for Data Analysis*](https://ggplot2-book.org/programming.html); [Chapter 4.6 of Peng, Kross, and Anderson's *Mastering Software Development in R*](https://bookdown.org/rdpeng/RProgDA/building-a-new-theme.html); LEA WANIEK Lea Waniek's blog post, [*Custom themes in ggplot2*](https://www.statworx.com/de/blog/custom-themes-in-ggplot2/), and Joey Stanley's blog post [*Custom Themes in ggplot2*](https://joeystanley.com/blog/custom-themes-in-ggplot2).
...
theme_pearl_earring <- function(light_color = "#E8DCCF", 
  ...
  theme(text       = element_text(color = light_color, family = my_family),
        ...
        line       = element_line(color = light_color),
...
```{r b13.1, echo = F}
# save(b13.1, file = "fits/b13.01.rda")
# rm(b13.1)
load("fits/b13.01.rda")
```
...
```{r b13.2, echo = F}
# save(b13.2, file = "fits/b13.02.rda")
# rm(b13.2)
load("fits/b13.02.rda")
```
...
```{r b13.3, echo = F}
# save(b13.3, file = "fits/b13.03.rda")
# rm(b13.3)
load("fits/b13.03.rda")
```
...
```{r b13.4, echo = F}
# save(b13.4, file = "fits/b13.04.rda")
# rm(b13.4)
load("fits/b13.04.rda")
```
...
```{r b13.6, echo = F}
# save(b13.6, file = "fits/b13.06.rda")
# rm(b13.6)
load("fits/b13.06.rda")
```
...
```{r b13.7, echo = F}
# save(b13.7, file = "fits/b13.07.rda")
# rm(b13.7)
load("fits/b13.07.rda")
```
...
That is, we used the brms default prior for $lscale$. In a [GitHub exchange](https://github.com/ASKurz/Statistical_Rethinking_with_brms_ggplot2_and_the_tidyverse/issues/8), Bürkner pointed out that brms uses special priors for $lscale$ parameters based on [Michael Betancourt](https://twitter.com/betanalpha)'s [vignette on the topic](https://betanalpha.github.io/assets/case_studies/gp_part3/part3.html). Though it isn't included in this document, I also ran the model with the `cauchy(0, 1)` prior and the results were quite similar. So the big discrepancy between our model and the one in the text isn't based on that prior.
...
```{r m13.7, echo = F}
# save(m13.7, file = "fits/m13.07.rda")
# rm(m13.7)
load("fits/m13.07.rda")
```
...
```{r b13.8, echo = F}
# save(b13.8, file = "fits/b13.08.rda")
# rm(b13.8)
load("fits/b13.08.rda")
```
...
```{r b13.9, echo = F}
# save(b13.9,  file = "fits/b13.09.rda")
# save(b13.10, file = "fits/b13.10.rda")
# rm(b13.9)
# rm(b13.10)
load("fits/b13.09.rda")
load("fits/b13.10.rda")
```
...
Here we'll practice our `tidybayes::spread_draws()` skills from the end of the last chapter to see what the random intercepts from the full model look like in a coefficient plot.
...
rm(d, rho, sigma, theme_pearl_earring, vary_effects, a, b, cov_ab, islandsDistMatrix, mu, n_cafes, n_visits, neff, ratios_cp, n_sim, r_1, r_2, r_4, sigma_a, sigma_b, sigmas, post, b13.1, p1, p2, b13.2, b13.3, b13.4, b13.6, b12.5, partially_pooled_params, un_pooled_params, params, partially_pooled_estimates, un_pooled_estimates, estimates, d_mat, k, i, j, nd, f, b13.7, m13.7, post_b13.7, post_m13.7, prob_to_logit, funding, b13.8, b13.9, b13.10, tidy_rho)
```
