...
```{r b12.1, echo = F}
# save(b12.1, file = "fits/b12.01.rda")
# rm(b12.1)
load("fits/b12.01.rda")
```
...
Do you remember the dummy variables and interaction models from back in Chapter 6? This model is like one of those on steroids. It'll be instructive to take a look at their distributions in density plots. We'll plot them in both their log-odds and probability metrics. 
...
```{r b12.2, echo = F}
# save(b12.2, file = "fits/b12.02.rda")
# rm(b12.2)
load("fits/b12.02.rda")
```
...
```{r b12.2b, echo = F}
# save(b12.2b, file = "fits/b12.02b.rda")
# rm(b12.2b)
load("fits/b12.02b.rda")
```
...
```{r b12.3, echo = F}
# save(b12.3, file = "fits/b12.03.rda")
# rm(b12.3)
load("fits/b12.03.rda")
```
...
```{r b12.3_new, echo = F}
# save(b12.3_new, file = "fits/b12.03_new.rda")
# rm(b12.3_new)
load("fits/b12.03_new.rda")
```
...
```{r b12.4, echo = F}
# save(b12.4, file = "fits/b12.04.rda")
# rm(b12.4)
load("fits/b12.04.rda")
```
...
```{r b12.5, echo = F}
# save(b12.5, file = "fits/b12.05.rda")
# rm(b12.5)
load("fits/b12.05.rda")
```
...
Happily, our `n_eff` values are generally quite higher than those McElreath presented in the text. The good folks at Stan have been working hard to improve the software. 
...
We might make the coefficient plot of Figure 12.4.a with `stanplot()`.
...
This section just hints at a historical software difficulty. In short, it's not uncommon to have a theory-based model that includes multiple sources of clustering (i.e., requiring many `( <varying parameter(s)> | <grouping variable(s)> )` parts in the model). This can make for all kinds of computational difficulties and result in software error messages, inadmissible solutions, and so on. One of the practical solutions to difficulties like these has been to simplify the statistical models by removing some of the clustering terms. Even though such simpler models were not the theory-based ones, at least they yielded solutions. Nowadays, Stan (via brms or otherwise) is making it easier to fit the full theoretically-based model. To learn more about this topic, check out this nice blog post by [Michael Frank](https://twitter.com/mcxfrank), [*Mixed effects models: Is it time to go Bayesian by default?*](http://babieslearninglanguage.blogspot.com/2018/02/mixed-effects-models-is-it-time-to-go.html). Make sure to check out the discussion in the comments section, which includes all-stars like Bürkner and [Douglas Bates](http://pages.stat.wisc.edu/~bates/). You can get more context for the issue from [this (2013) paper](https://www.sciencedirect.com/science/article/pii/S0749596X12001180) by Barr, Levy, Scheepers, and Tily. 
...
```{r b12.6, echo = F}
# save(b12.6, file = "fits/b12.06.rda")
# rm(b12.6)
load("fits/b12.06.rda")
```
...
```{r b12.7, echo = F}
# save(b12.7, file = "fits/b12.07.rda")
# rm(b12.7)
load("fits/b12.07.rda")
```
...
```{r b12.8, echo = F}
# save(b12.8, file = "fits/b12.08.rda")
# rm(b12.8)
load("fits/b12.08.rda")
```
