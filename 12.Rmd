...
First, we took one random draw from a normal distribution with a mean of the first row in `post[, 1]` and a standard deviation of the value from the first row in `post[, 2]`, and passed it through the `inv_logit_scaled()` function. By replacing the `1` with `nrow(post)`, we do this `nrow(post)` times (i.e., 12,000). Our orange density, then, is the summary of that process.
...
When McElreath [lectured on this topic in 2015](https://www.youtube.com/watch?v=82TaniPgzQc&t=2048s&frags=pl%2Cwn), he traced partial pooling to statistician [Charles M. Stein](http://bulletin.imstat.org/2017/05/obituary-charles-m-stein-1920-2016/). In 1977, Efron and Morris wrote the now classic paper, [*Stein’s Paradox in Statistics*](http://statweb.stanford.edu/%7Eckirby/brad/other/Article1977.pdf), which does a nice job breaking down why partial pooling can be so powerful. One of the primary examples they used in the paper was of 1970 batting average data. If you’d like more practice seeing how partial pooling works--or if you just like baseball--, check out my project on that example, [*James-Stein and Bayesian partial pooling*](https://github.com/ASKurz/James-Stein-and-Bayesian-partial-pooling).
...
You can get the LOO version of the `p_waic`, the `p_loo`, like so.
...
And if you peek at the structure of the loo objects, you'll see you can extract the `p_loo` values directly with something like `l.b12.5$ estimates["p_loo", 1]`. The results are quite similar to those in the text.
...
f1 <-
  fitted(b12.7,
         ...
         scale = "response") %>% 
  as_tibble() %>% 
  rename(`chimp 1's average probability of pulling left` = V1,
         `chimp 2's average probability of pulling left` = V2,
         `chimp 3's average probability of pulling left` = V3,
         `chimp 4's average probability of pulling left` = V4,
         `chimp 5's average probability of pulling left` = V5,
         `chimp 6's average probability of pulling left` = V6,
         `chimp 7's average probability of pulling left` = V7)

str(f1)
```

This scales reasonably well, with the sole exception of the `rename()` code. One great alternative is with `purr::set_names()` and some tricky help from `paste()`.
...
f1 <-
  fitted(b12.7,
         ...
         scale = "response") %>% 
  as_tibble() %>% 
  set_names(paste("chimp ", 1:7, "'s average probability of pulling left", sep = ""))

str(f1)
```

That was super slick, but might not work as work as well if the vectors you wanted to rename didn’t follow a serial order, like ours. If you’re willing to pay with a few more lines of wrangling code, this method is more general, but still scalable.
...
First, notice `tidybayes::spread_draws()` took the model fit itself, `b12.7`, as input. No need for `posterior_samples()`. Now, notice we fed two additional arguments into it. By the first argument, we that requested `spead_draws()` extract the posterior samples for the `b_Intercept`. By the second argument, `r_actor[actor,]`, we instructed `spead_draws()` to extract all the random effects for the `actor` variable. Also notice how within the brackets `[]` we specified `actor`, which then became the name of the column in the output that indexed the levels of the grouping variable `actor`. By default, the code returns the posterior samples for all the levels of `actor`. However, had we only wanted those from chimps #1 and #3, we might use typical tidyverse-style indexing. E.g.,
...
```{r, warning = F, echo = F}
rm(d, b12.1, b12.2, post, invlogit, postMdn, b12.2.e, a, sigma, n_ponds, dsim, b12.3, n_iter, p_partpool, dfline, two_gaussians, b12.4, b12.5, l.b12.4, l.b12.5, nd, chimp, chimp_2_fitted, chimp_2_d, my_fitted, chimp_5_my_fitted, chimp_5_d, post_average_actor, ran_ef, fix_ef, ran_and_fix_ef, marginal_effects, p1, p2, p3, b12.6, pred_12.6, c1, c2, c3, f1, f2, f3, s1, s2, s3)
```
