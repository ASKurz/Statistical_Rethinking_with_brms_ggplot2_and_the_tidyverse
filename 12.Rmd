...
Here we follow along with Mcelreath and "assign specific values representative of the actual tadpole data" (p. 366). However, out values will differ a little from his. Because he did not show a `set.seed()` line, we don’t know what seed was used to generate pseudo random draws from the `rnorm()` function. 
...
Here's the posterior distribution for $\sigma_\text{actor}$.
...
We can inspect the $\hat R$ and effective sample size values for all parameters like so.

```{r}
b12.4$fit
```

It appears this version of the output summary has not updated to use `Bulk_ESS` and `Tail_ESS`.
