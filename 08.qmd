...
# See also this for font management on a macOS
...
```{r, fig.width = 5.5, fig.height = 4.75, warning = F, message = F}
library(tidyverse)
...
## Easy HMC: ~~map2stan~~ `brm()` {#sec-Easy-HMC}
...
my_lower <- function(data, mapping, ...) {
  ggplot(data = data, mapping = mapping) + 
    geom_point(color = pomological_palette[3],
               shape = 1, size = 1/2, alpha = 1/6)
  }
...
```{r}
b8.1 <- add_criterion(b8.1, criterion = c("waic", "loo"))
```
...
$$\text{elpd} = \sum_{i = 1}^n \int p_t (\tilde y_i) \log p (\tilde y_i \mid y) d \tilde y_i,$$
...
### Checking the chain {#sec-Checking-the-chain}
...
mcmc_trace(post,
  ...
  labs(title = "My version of Figure 8.5.a.",
       subtitle = "These trace plots do not look like the fuzzy caterpillars\nwe usually hope for.") +
  scale_color_pomological() +
  theme_pomological_fancy(base_family = "Marck Script") +
...
```{r, fig.height = 4.0, fig.width = 7.25, warning = F}
# left
p1 <- post %>%
  select(b_Intercept) %>%
...
p1 + p2 + plot_annotation(
  title = "Prior (dashed) and posterior (solid) distributions for the\nmodel with weakly-informative priors, b8.3",
  theme = theme_pomological_fancy(base_family = "Marck Script"))
```  
...
tibble(a = rcauchy(n, location = 0, scale = 5),
  ...
  scale_color_manual(values = pomological_palette) +
