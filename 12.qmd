# Multilevel Models {#sec-Multilevel-Models}
...
Now fit this simple aggregated binomial model much like we practiced in @sec-Aggregated-binomial-Chimpanzees-again-condensed.
...
Do you remember the dummy variables models from back in @sec-Binary-categories? This model is like one of those on steroids. It'll be instructive to take a look at their distributions in density plots. We'll plot them in both their log-odds and probability metrics.
...
post_mdn <- tibble(Estimate = coef(b12.2, robust = T)$tank[, "Estimate", ]) %>%
...
>
> The model uses a logit link, and so the probability is defined by the [`plogis()`] function. (p. 367)
...
p_partpool <- tibble(Estimate = coef(b12.3)$pond[, "Estimate", ]) %>% 
...
The initial multilevel update from model `b10.4` from @sec-Prosocial-chimpanzees follows the statistical formula

\begin{align*}
\text{left\_pull}_i        & \sim \operatorname{Binomial}(n_i = 1, p_i) \\
\operatorname{logit}(p_i) & = \alpha + \color{blue}{\alpha_{\text{actor}_i}} + (\beta_1 + \beta_2 \text{condition}_i) \text{prosoc\_left}_i \\
...
```{r}
set.seed(12)
two_gaussians <- tibble(
  y1 =      rnorm(n = 1e4, mean = 10, sd = 1),
...
We can use the `summarise_draws()` function from the posterior package to compute the familiar diagnostic statistics for all parameters in the model.
...
### Two types of cluster {#sec-Two-types-of-cluster}

The full cross-classified statistical model follows the form

\begin{align*}
\text{left\_pull}_i        & \sim \operatorname{Binomial}(n_i = 1, p_i) \\
\operatorname{logit}(p_i) & = \alpha + \alpha_{\text{actor}_i} + \color{blue}{\alpha_{\text{block}_i}} + (\beta_1 + \beta_2 \text{condition}_i) \text{prosoc\_left}_i \\
...
```{r}
b12.4 <- add_criterion(b12.4, criterion = "loo")
b12.5 <- add_criterion(b12.5, criterion = "loo")
...
# the random effects
set.seed(12.42)
ran_ef <- tibble(random_effect = rnorm(n = 1000, mean = 0, sd = post$sd_actor__Intercept)) %>% 
...
# combine them
ran_and_fix_ef <- bind_cols(ran_ef, fix_ef) %>%
  mutate(intercept = fixed_effect + random_effect) %>%
  mutate(prob = plogis(intercept))

# to simplify things, we'll reduce them to summaries
(
  marginal_effects <- ran_and_fix_ef %>%
  group_by(condition) %>%
...
### Focus and multilevel prediction {#sec-Focus-and-multilevel-prediction}
...
\begin{align*}
\text{total\_tools}_i  & \sim \operatorname{Poisson}(\mu_i) \\
...
```{r}
nd <- tibble(population = seq(from = 1000, to = 400000, by = 5000),
...
For a detailed discussion on this way of using `brms::predict()`, see [Andrew MacDonald](https://github.com/aammd)'s [great blogpost on this very figure](https://thestudyofthehousehold.netlify.app/2018/02/13/2018-02-13-easily-made-fitted-and-predicted-values-made-easy/). Here's what we've been working for.
...
> The envelope of predictions is a lot wider here than it was back in @sec-Counting-and-Classification. This is a consequene of the varying intercepts, combined with the fact that there is much more variation in the data than a pure-Poisson model anticipates. (p. 384)

## ~~Summary~~ Bonus: Put your random effects to work {#sec-Put-your-random-effects-to-work}
...
\begin{align*}
\text{pulled\_left}_i      & \sim \operatorname{Binomial}(n_i = 1, p_i) \\
...
```{r, warning = F}
as_draws_df(b12.7) %>% 
  transmute(`chimp 1's average probability of pulling left` = (b_Intercept + `r_actor[1,Intercept]`) %>% plogis()) %>% 
  head()
```
...
By default, we get the familiar summaries for mean performances for each of our seven chimps. These, of course, are in the log-odds metric and simply tacking on `plogis()` isn't going to fully get the job done if we want posterior standard deviations. So to get things in the probability metric, we'll want to first set `summary = F` in order to work directly with un-summarized samples and then wrangle quite a bit. Part of the wrangling challenge is because `coef()` returns a list, rather than a data frame. With that in mind, the code for our first task of getting the posterior draws for the `actor`-level estimates from the `b12.7` model looks like so.

```{r}
c1 <- coef(b12.7, summary = F)$actor[, , ] %>%
  data.frame() %>% 
  gather() %>% 
...
f1 <- fitted(b12.7,
             ...
             # `plogis()` did with the other two methods
             scale = "response") %>% 
  data.frame() %>% 
...
s1 <- b12.7 %>%
  spread_draws(b_Intercept, r_actor[actor,]) %>% 
  mutate(p = (b_Intercept + r_actor) %>% plogis()) %>% 
  select(.draw, actor, p) %>% 
  ungroup() %>% 
...
s3 <- b12.8 %>%
  ...
  filter(block == 1) %>% 
  mutate(p = (b_Intercept + r_actor + r_block) %>% plogis()) %>% 
  select(.draw, actor, p) %>% 
  ungroup() %>% 
  mutate(actor = str_c("chimp ", actor, "'s average probability of pulling left")) %>% 
  spread(value = p, key = actor)
