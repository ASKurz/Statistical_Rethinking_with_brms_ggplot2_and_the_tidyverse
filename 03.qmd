...
```{r, warning = F}
(
  point_estimates <- bind_rows(samples %>% mean_qi(p_grid),
...
With brms, we'll fit the primary model of $w = 6$ and $n = 9$ much like we did in @sec-Markov-chain-Monte-Carlo.
...
We'll learn more about the beta distribution in @sec-Monsters-and-Mixtures. For now, here's the posterior summary for `b_Intercept`, the probability of a "w".
