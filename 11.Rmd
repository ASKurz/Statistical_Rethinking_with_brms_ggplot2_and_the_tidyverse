...
Let's get the `Trolley` data from rethinking (see [Cushman, Young & Hauser, 2006](https://journals.sagepub.com/doi/pdf/10.1111/j.1467-9280.2006.01834.x)).
...
```{r b11.1, echo = F}
# save(b11.1, file = "fits/b11.01.rda")
# rm(b11.1)
load("fits/b11.01.rda")
```
...
What McElreath's `m11.1stan` summary termed `cutpoints[k]`, ours termed `Intercept[k]`. In both cases, these are the $\alpha_k$ parameters from the equations, above. The summaries look like those in the text, the $\hat R $ values are great, and both measures of effective sample size are reasonably high. The model looks good.
...
```{r b11.1b, echo = F}
# save(b11.1b, file = "fits/b11.01b.rda")
# rm(b11.1b)
load("fits/b11.01b.rda")
```
...
This yielded a mean estimate of 4.2, much like our `average_outcome_value`, above.
...
So the rule is we *subtract the linear model from each interecept*. "This way, a positive $\beta$ value indicates that an increase in the predictor variable $x$ results in an increase in the average response" (p. 338). Let's fit our multivariable models.

```{r b11.2, echo = F}
# save(b11.2, file = "fits/b11.02.rda")
# save(b11.3, file = "fits/b11.03.rda")
# rm(b11.2)
# rm(b11.3)
load("fits/b11.02.rda")
load("fits/b11.03.rda")
```
...
p1 + p2 + p3 + plot_annotation(title    = "Posterior predictions of the interaction model:",
                               subtitle = "Here we're focusing on the parameters.")
```

If you'd like to learn more about these kinds of models and how to fit them in brms, check out Bürkner and Vuorre's [*Ordinal Regression Models in Psychology: A Tutorial*](https://psyarxiv.com/x8swp/) and its [repository](https://osf.io/cu8jv/) on the Open Science Framework.
...
Our handy homemade `make_alternative_data()` function works very much like its predecessor. Before we put it to work, how might simplify our upcoming ggplot2 code. Did you notice how the last three plots, those three subplots for Figure 11.3, were all basically the same with slightly different input. Instead of copy/pasting all that plot code, we can wrap the bulk of it into a custom plotting function we'll call `geom_figure11.3()`.
...
If your new to making functions with ggplot2 components, check out [Chapter 16](https://ggplot2-book.org/programming.html) of Wickham's [*ggplot2: Elegant Graphics for Data Analysis*](https://ggplot2-book.org/index.html). He'll walk you through all the steps.
...
```{r b11.4, echo = F}
# save(b11.4, file = "fits/b11.04.rda")
# rm(b11.4)
load("fits/b11.04.rda")
```
...
```{r b11.5, echo = F}
# save(b11.5, file = "fits/b11.05.rda")
# rm(b11.5)
load("fits/b11.05.rda")
```
...
McElreath didn't give an example of negative-binomial regression in the text. This can be a useful but somewhat perplexing model type, so let's take a detour for an extended example with the `UCBadmit` data. As in other examples, we will be modeling the `admit` rates.

```{r b11.6, echo = F}
# save(b11.6, file = "fits/b11.06.rda")
# rm(b11.6)
load("fits/b11.06.rda")
```
...
ggplot(data = tibble(x = seq(from = 0, to = 60, by = .1)),
  ...
  ggtitle(expression(paste("brms default ", gamma, "(0.01, 0.01) shape prior"))) +
  theme_hc() +
  theme(plot.background = element_rect(fill = "grey92"))
```
...
Because this model had no predictors, we have the similar posterior-predictive distribution for each case. It's important, however, to be very clear of what these posterior-predictive distributions are of. They are not of the data, per se. Let's look back at the text:

> A **negative-binomial** model, more useful called a **gamma-Poisson** model, assumes that each Poisson count observation has its own rate. It estimates the shape of the gamma distribution to describe the Poisson rates across cases. (p. 350, **emphasis** in the original)

As a reminder, the "rate" for the Poisson distribution is just another word for the mean, also called $\lambda$. So unlike a simple Poisson model where we use the individual cases to estimate one overall $\lambda$, here we're presuming each case has it's own $\lambda_i$. So there are 12 $\lambda_i$ values that generated our data and if we look at those $\lambda_i$ values on the whole, they can be described with a gamma distribution.
...
There is a difficulty, however, if we would like to use McElreath's `rethinking::dgamma2()` to visualize what gamma distributions based on those parameters might look like. The `dgamma2()` function has arguments for the mean and the scale, not the shape. As it turns out, we can express the mean of the gamma density in terms of the shape and scale with the equation
...
And much like we did in Figure 11.5 for the beta binomial model, here is the plot of that posterior mean gamma distribution accompanied by an additional 100 combinations of $\mu$ and $\theta$ values sampled from the posterior.
...
post %>% 
  ...
  stat_function(fun = rethinking::dgamma2,
                ...
                size = 1.5) +
  scale_y_continuous(NULL, breaks = NULL) +
  xlab(expression(paste("distribution of ", lambda, " values"))) +
  coord_cartesian(xlim = 0:500,
                  ylim = c(0, 0.01)) +
  theme_hc() +
  theme(plot.background = element_rect(fill = "grey92"))
...
post %>% 
  ...
  geom_density(color = "transparent", 
              fill = canva_pal("Green fields")(4)[2]) +
  scale_y_continuous(NULL, breaks = NULL) +
  xlab(expression(paste("the variance (i.e., ", alpha, theta^2, ")"))) +
...
```{r, echo = F}
rm(d, logit, d_plot, b11.1, b11.1b, logistic, dordlogit, pk, explicit_example,  b11.2, b11.3, nd, max_iter, make_Figure_11.3_data, make_alternative_data, geom_figure11.3, p1, p2, p3, prob_drink, rate_work, n, drink, y, b11.4, lambda_b11.4, p_b11.4, pbar, theta, betaABfromMeanKappa, beta_binomial2, beta_binomial2_lpmf,  beta_binomial2_rng, stanvars, dzip, stan_funs, b11.5, lines, log_lik_beta_binomial2, predict_beta_binomial2, fitted_beta_binomial2, post, mu, b11.6, p, alpha)
```
