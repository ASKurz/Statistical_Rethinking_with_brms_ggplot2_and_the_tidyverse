...
```{r b5.1, echo = F}
# save(b5.1, file = "fits/b05.01.rda")
# rm(b5.1)
load("fits/b05.01.rda")
```
...
```{r b5.2, echo = F}
# save(b5.2, file = "fits/b05.02.rda")
# rm(b5.2)
load("fits/b05.02.rda")
```
...
```{r b5.3, echo = F}
# save(b5.3, file = "fits/b05.03.rda")
# rm(b5.3)
load("fits/b05.03.rda")
```
...
```{r b5.4, echo = F}
# save(b5.4, file = "fits/b05.04.rda")
# rm(b5.4)
load("fits/b05.04.rda")
```
...
```{r b5.4b, echo = F}
# save(b5.4b, file = "fits/b05.04b.rda")
# rm(b5.4b)
load("fits/b05.04b.rda")
```
...
```{r b5.0_spur, echo = F}
# save(b5.0_spur, file = "fits/b05.00_spur.rda")
# rm(b5.0_spur)
load("fits/b05.00_spur.rda")
```
...
```{r b5.5, echo = F}
# save(b5.5, file = "fits/b05.05.rda")
# rm(b5.5)
load("fits/b05.05.rda")
```
...
```{r b5.6, echo = F}
# save(b5.6, file = "fits/b05.06.rda")
# rm(b5.6)
load("fits/b05.06.rda")
```
...
```{r b5.7, echo = F}
# save(b5.7, file = "fits/b05.07.rda")
# rm(b5.7)
load("fits/b05.07.rda")
```
...
```{r b5.0_both, echo = F}
# save(b5.0_both, file = "fits/b05.00_both.rda")
# save(b5.0_pos, file = "fits/b05.00_pos.rda")
# save(b5.0_neg, file = "fits/b05.00_neg.rda")
# rm(b5.0_both)
# rm(b5.0_pos)
# rm(b5.0_neg)
load("fits/b05.00_both.rda")
load("fits/b05.00_pos.rda")
load("fits/b05.00_neg.rda")
```
...
```{r b5.8, echo = F}
# save(b5.8, file = "fits/b05.08.rda")
# rm(b5.8)
load("fits/b05.08.rda")
```
...
Here's our coefficient plot using `brms::stanplot()`.
...
```{r b5.9, echo = F}
# save(b5.9, file = "fits/b05.09.rda")
# rm(b5.9)
load("fits/b05.09.rda")
```
...
We might also use `tidybayes::stat_pointintervalh()` to compare the posterior of `b_leg_left` from `b5.9` to the joint posterior `b_leg_left + b_leg_right` from `b5.8`. For kicks, we'll use three levels of posterior intervals.

```{r, fig.width = 6, fig.height = 1, warning = F, message = F}
bind_cols(
  post %>% transmute(`b_leg_left + b_leg_right` = b_leg_left + b_leg_right),
  posterior_samples(b5.9) %>% transmute(`only b_leg_left` = b_leg_left)
) %>% 
  gather() %>% 

  ggplot(aes(x = value, y = key)) +
  stat_pointintervalh(.width = c(.5, .8, .95),
                      color = "firebrick") +
  labs(x = NULL,
       y = NULL) +
  theme_bw() +
...
```{r b5.10, echo = F}
# save(b5.10, file = "fits/b05.10.rda")
# save(b5.11, file = "fits/b05.11.rda")
# rm(b5.10)
# rm(b5.11)
load("fits/b05.10.rda")
load("fits/b05.11.rda")
```
...
```{r b5.12, echo = F}
# save(b5.12, file = "fits/b05.12.rda")
# rm(b5.12)
load("fits/b05.12.rda")
```
...
```{r b5.13, echo = F}
# save(b5.13, file = "fits/b05.13.rda")
# rm(b5.13)
load("fits/b05.13.rda")
```
...
```{r b5.14, echo = F}
# save(b5.14, file = "fits/b05.14.rda")
# rm(b5.14)
load("fits/b05.14.rda")
```
...
```{r b5.15, echo = F}
# save(b5.15, file = "fits/b05.15.rda")
# rm(b5.15)
load("fits/b05.15.rda")
```
...
```{r b5.15b, echo = F}
# save(b5.15b, file = "fits/b05.15b.rda")
# rm(b5.15b)
load("fits/b05.15b.rda")
```
...
```{r b5.16, echo = F}
# save(b5.16, file = "fits/b05.16.rda")
# rm(b5.16)
load("fits/b05.16.rda")
```
...
```{r b5.16_alt, echo = F}
# save(b5.16_alt, file = "fits/b05.16_alt.rda")
# rm(b5.16_alt)
load("fits/b05.16_alt.rda")
```
...
```{r, echo = F}
rm(d, b5.1, nd, f, b5.2, b5.3, post, b5.4, r, text, b5.4b, n, p1, p2, p3, b5.0_spur, b5.5, dcc, b5.6, b5.7, rho, b5.0_both, b5.0_pos, b5.0_neg, b5.8, b5.9, b5.10, b5.11, b5.12, my_diag, my_lower, sim_coll, n_seed, n_rho, b5.13, b5.14, b5.15, b5.15b, b5.16, b5.16_alt)
```
