...
The posterior is a combination of the prior and the likelihood. And when the prior is flat across the parameter space, the posterior is just the likelihood re-expressed as a probability.
...
![](04_files/figure-markdown_github/unnamed-chunk-27-1.png)
...
```
    ...
    ## Intercept   154.61      0.41   153.82   155.45       3110 1.00
    ## 
    ## Family Specific Parameters: 
    ##       Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
    ## sigma     7.74      0.30     7.18     8.34       3100 1.00
    ## 
...
```
    ...
    ## b_Intercept   154.61    0.01 0.41   153.82   154.32   154.60   154.88   155.45  3110    1
    ## sigma           7.74    0.01 0.30     7.18     7.54     7.72     7.94     8.34  3100    1
    ## lp__        -1227.54    0.03 1.01 -1230.21 -1227.92 -1227.22 -1226.81 -1226.54  1568    1
    ## 
    ## Samples were drawn using NUTS(diag_e) at Fri Sep 21 21:39:31 2018.
...
```
    ...
    ## Intercept   154.61      0.41   153.95   155.28       3110 1.00
    ## 
    ## Family Specific Parameters: 
    ##       Estimate Est.Error l-89% CI u-89% CI Eff.Sample Rhat
    ## sigma     7.74      0.30     7.27     8.22       3100 1.00
    ## 
    ## Samples were drawn using sampling(NUTS). For each parameter, Eff.Sample 
    ## is a crude measure of effective sample size, and Rhat is the potential 
    ## scale reduction factor on split chains (at convergence, Rhat = 1).
...
```
    ...
    ## Population-Level Effects: 
    ##           Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
    ## Intercept   177.87      0.10   177.67   178.07       3217 1.00
    ## 
    ## Family Specific Parameters: 
    ##       Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
    ## sigma    24.61      0.94    22.89    26.45       1489 1.00
    ## 
    ## Samples were drawn using sampling(NUTS). For each parameter, Eff.Sample 
    ## is a crude measure of effective sample size, and Rhat is the potential 
    ## scale reduction factor on split chains (at convergence, Rhat = 1).
...
```
    ...
    ##              b_Intercept        sigma
    ## b_Intercept  0.171250499 -0.005917398
    ## sigma       -0.005917398  0.088383749
...
```
    ...
    ##             b_Intercept       sigma
    ## b_Intercept  1.00000000 -0.04809818
    ## sigma       -0.04809818  1.00000000
...
```
    ...
    ## 1    154.5883 7.783068 -1226.541
    ## 2    155.0592 7.737251 -1227.120
    ## 3    155.2591 8.004362 -1228.132
    ## 4    154.2575 7.771651 -1226.888
    ## 5    154.4029 7.802011 -1226.676
    ## 6    154.6767 8.029784 -1227.060
...
```
    ...
    ##   b_Intercept        sigma      
    ##  Min.   :153.2   Min.   :6.816  
    ##  1st Qu.:154.3   1st Qu.:7.538  
    ##  Median :154.6   Median :7.721  
    ##  Mean   :154.6   Mean   :7.736  
...
```
    ...
    ##                    50%       2.5%        75%
    ## b_Intercept 154.603707 153.815245 154.884745
    ## sigma         7.720563   7.176119   7.938939
...
```
    ...
    ## b_Intercept   154.607136 0.4138242   153.815245   155.448542
    ## sigma           7.735537 0.2972940     7.176119     8.342962
    ## lp__        -1227.537916 1.0079741 -1230.205072 -1226.541840
...
```
    ...
    ## 2 sigma         7.74   7.18   8.34   0.95 mean   qi
...
-   *α* ∼ Normal(156, 100): `prior(normal(156, 100), class = Intercept`
...
```
    ...
    ##                Estimate  Est.Error        Q2.5       Q97.5
    ## b_Intercept 113.8098466 1.90316243 110.0931234 117.3675771
    ## b_weight      0.9065417 0.04195566   0.8285624   0.9881767
    ## sigma         5.1013030 0.19884899   4.7287466   5.5188076
...
```
    ...
    ##                Estimate  Est.Error        Q2.5       Q97.5
    ## b_Intercept 154.5973662 0.27878726 154.0439025 155.1290594
    ## b_weight_c    0.9065825 0.04375661   0.8188434   0.9899186
    ## sigma         5.1113476 0.19514401   4.7484510   5.5033863
...
```
    ...
    ## b_Intercept        1.00       0.00  0.02
    ## b_weight_c         0.00       1.00 -0.07
    ## sigma              0.02      -0.07  1.00
...
```
    ...
    ## 1    115.2447 0.8668875 5.363728 -1084.178
    ## 2    114.0342 0.9077231 4.855305 -1083.355
    ## 3    113.1116 0.9219245 4.909806 -1082.549
    ## 4    112.5973 0.9327167 4.838990 -1083.149
    ## 5    115.2409 0.8732348 5.397566 -1083.579
...
```
    ...
    ## 1 159.1369 158.6041 159.6985   0.89   mean       hdi
    ## 2 159.1369 158.4832 159.8054   0.95   mean       hdi
...
![](04_files/figure-markdown_github/unnamed-chunk-68-1.png)
...
```
    ...
    ##      <dbl>     <dbl> <dbl> <dbl>  <dbl>
    ## 1     136.     0.877  135.  138.     25
    ## 2     137.     0.837  136.  139.     26
    ## 3     138.     0.797  137.  140.     27
    ## 4     139.     0.758  138.  141.     28
    ## 5     140.     0.719  139.  141.     29
    ## 6     141.     0.680  140.  142.     30
...
```
    ...
    ##   Estimate Est.Error  Q2.5 Q97.5 weight
    ##      <dbl>     <dbl> <dbl> <dbl>  <dbl>
    ## 1     136.      5.17  126.  147.     25
    ## 2     137.      5.13  127.  148.     26
    ## 3     138.      5.21  128.  149.     27
...
```
    ...
    ## Population-Level Effects: 
    ##             Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
    ## Intercept     146.66      0.38   145.92   147.42       3562 1.00
    ## weight_s       21.41      0.29    20.81    21.97       3565 1.00
    ## Iweight_sE2    -8.41      0.28    -8.96    -7.85       3627 1.00
    ## 
    ## Family Specific Parameters: 
    ##       Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
    ## sigma     5.77      0.18     5.43     6.13       3642 1.00
    ## 
    ## Samples were drawn using sampling(NUTS). For each parameter, Eff.Sample 
    ## is a crude measure of effective sample size, and Rhat is the potential 
    ## scale reduction factor on split chains (at convergence, Rhat = 1).
