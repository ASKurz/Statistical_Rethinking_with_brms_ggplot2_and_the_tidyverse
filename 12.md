...
```
    ...
    ## b12.1         200.86 9.35
    ## b12.2         201.03 7.26
    ## b12.1 - b12.2  -0.17 4.52

``` r
loo(b12.1, b12.2)
```

    ## Warning: Found 45 observations with a pareto_k > 0.7 in model 'b12.1'.
...
```
    ...
    ## b12.1          324.12 13.30
    ## b12.2          263.47 13.04
    ## b12.1 - b12.2   60.65  8.45

The *K*-fold cross-validation difference of 61, with a standard error around 8, suggests that model `b12.2` is the clear favorite relative to `b12.1`. For more on the `kfold()` function, see the [brms reference manual](https://cran.r-project.org/web/packages/brms/brms.pdf).
...
```
    ...
    ##  1    2.07      0.849  0.563  4.04      10 no    big       9      0.9
    ##  2    2.95      1.09   1.20   5.55      10 no    big      10      1  
    ##  3    0.973     0.660 -0.239  2.38      10 no    big       7      0.7
    ##  4    2.95      1.10   1.18   5.54      10 no    big      10      1  
    ##  5    2.06      0.829  0.615  4.00      10 no    small     9      0.9
    ##  6    2.07      0.851  0.597  4.01      10 no    small     9      0.9
    ##  7    2.96      1.09   1.19   5.49      10 no    small    10      1  
    ##  8    2.06      0.839  0.631  4.01      10 no    small     9      0.9
    ##  9   -0.175     0.603 -1.39   1.01      10 pred  big       4      0.4
    ## 10    2.05      0.843  0.561  4.05      10 pred  big       9      0.9
...
```
    ...
    ## sd(Intercept)     1.61      0.21     1.25     2.08       2048 1.00
    ## 
    ## Population-Level Effects: 
    ##           Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
    ## Intercept     1.31      0.25     0.82     1.81       1615 1.00
    ## 
...
| actor     |   mean| 95% CIs          |
|:----------|------:|:-----------------|
| chimp \#1 |  -0.71| \[-1.26, -0.2\]  |
| chimp \#2 |   4.64| \[2.56, 8.65\]   |
| chimp \#3 |  -1.02| \[-1.58, -0.48\] |
...
```
    ...
    ##               Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
    ## sd(Intercept)     2.28      0.95     1.12     4.66       5472 1.00
    ## 
    ## ~block (Number of levels: 6) 
    ##               Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
    ## sd(Intercept)     0.22      0.18     0.01     0.68       8318 1.00
    ## 
    ## Population-Level Effects: 
    ##                       Estimate Est.Error l-95% CI u-95% CI Eff.Sample Rhat
    ## Intercept                 0.45      0.96    -1.33     2.47       5304 1.00
    ## prosoc_left               0.83      0.26     0.31     1.34      15476 1.00
    ## prosoc_left:condition    -0.14      0.30    -0.73     0.45      15535 1.00
    ## 
    ## Samples were drawn using sampling(NUTS). For each parameter, Eff.Sample 
    ## is a crude measure of effective sample size, and Rhat is the potential 
    ## scale reduction factor on split chains (at convergence, Rhat = 1).
...
```
    ...
    ##   Estimate Est.Error  Q2.5 Q97.5
    ## 1    -1.18      0.97 -3.23  0.62
    ## 2     4.17      1.65  1.79  8.11
    ## 3    -1.48      0.97 -3.54  0.33
    ## 4    -1.48      0.98 -3.53  0.32
    ## 5    -1.17      0.97 -3.22  0.63
    ## 6    -0.22      0.97 -2.26  1.58
    ## 7     1.32      0.99 -0.73  3.22
...
```
    ...
    ##             Estimate         SE
    ## elpd_loo -265.768828  9.7477565
    ## p_loo       8.201526  0.4370585
    ## looic     531.537655 19.4955131
...
```
    ...
    ##            Estimate         SE
    ## elpd_loo -266.38556  9.8474743
    ## p_loo      10.44037  0.5324451
    ## looic     532.77112 19.6949487
...
```
    ...
    ##      <dbl>     <dbl> <dbl> <dbl> <fct>    
    ## 1    0.981   0.0197  0.928 1.000 0/0      
    ## 2    0.991   0.00954 0.965 1.000 1/0      
    ## 3    0.981   0.0197  0.928 1.000 0/1      
...
```
    ...
    ## Observations: 16,000
    ## Variables: 12
    ## $ b_Intercept               <dbl> 0.379163300, 1.909679264, 0.16073906...
    ## $ b_prosoc_left             <dbl> 0.5111643, 0.7970544, 0.7196387, 0.7...
    ## $ `b_prosoc_left:condition` <dbl> -0.08929162, -0.16655019, -0.0440546...
    ## $ sd_actor__Intercept       <dbl> 1.397075, 2.041141, 1.830859, 2.2764...
    ## $ `r_actor[1,Intercept]`    <dbl> -0.7194780, -2.4739624, -0.6787636, ...
    ## $ `r_actor[2,Intercept]`    <dbl> 3.308124, 2.508650, 4.124420, 2.4686...
    ## $ `r_actor[3,Intercept]`    <dbl> -1.1442557, -2.9674083, -0.7149021, ...
    ## $ `r_actor[4,Intercept]`    <dbl> -0.9492540, -2.5683315, -1.2896389, ...
    ## $ `r_actor[5,Intercept]`    <dbl> -0.9551195, -2.1914301, -0.8804910, ...
    ## $ `r_actor[6,Intercept]`    <dbl> 0.07735738, -1.57496576, 0.34505746,...
    ## $ `r_actor[7,Intercept]`    <dbl> 1.4187318, -0.4422322, 2.1356083, 0....
    ## $ lp__                      <dbl> -281.9081, -282.0868, -281.9272, -28...
...
```
    ...
    ## 1 0/0        0.331  0.222  0.451   0.95 mean   qi       
    ## 2 1/0        0.526  0.381  0.668   0.95 mean   qi       
    ## 3 0/1        0.331  0.222  0.451   0.95 mean   qi       
...
```
    ...
    ##   key       m    ll    ul
    ##   <fct> <dbl> <dbl> <dbl>
    ## 1 0/0   0.590 0.346 0.826
    ## 2 1/0   0.746 0.543 0.918
    ## 3 0/1   0.590 0.346 0.826
    ## 4 1/1   0.723 0.504 0.908
...
```
    ...
    ##   condition     m     ll    ul
    ##   <fct>     <dbl>  <dbl> <dbl>
    ## 1 0/0       0.552 0.0783 0.972
    ## 2 1/0       0.666 0.167  0.989
    ## 3 0/1       0.552 0.0783 0.972
    ## 4 1/1       0.647 0.138  0.986
...
```
    ...
    ## 1    0.590     0.183 0.346 0.826           0 0/0      
    ## 2    0.746     0.152 0.543 0.918           1 1/0      
    ## 3    0.590     0.183 0.346 0.826           0 0/1      
    ## 4    0.723     0.159 0.504 0.908           1 1/1
...
![](12_files/figure-markdown_github/unnamed-chunk-59-1.png)
...
```
    ...
    ##   Estimate Est.Error    Q10   Q90 prosoc_left condition
    ##      <dbl>     <dbl>  <dbl> <dbl>       <dbl> <fct>    
    ## 1    0.559     0.329 0.0719 0.971           0 0/0      
    ## 2    0.668     0.310 0.147  0.987           1 1/0      
    ## 3    0.559     0.329 0.0719 0.971           0 0/1      
    ## 4    0.651     0.314 0.130  0.985           1 1/1
...
```
    ...
    ## Observations: 80
    ## Variables: 10
    ## $ Estimate   <dbl> 19.80667, 30.99056, 36.35400, 40.02644, 43.12411, 4...
    ## $ Est.Error  <dbl> 9.859101, 13.291555, 15.417652, 16.748679, 18.15826...
    ## $ Q1.5       <dbl> 5.000, 10.000, 12.000, 14.000, 15.000, 15.000, 17.0...
    ## $ Q5.5       <dbl> 8, 15, 18, 20, 21, 22, 23, 24, 25, 25, 26, 26, 27, ...
    ## $ Q16.5      <dbl> 12, 20, 24, 26, 28, 30, 31, 33, 33, 35, 35, 36, 36,...
    ## $ Q83.5      <dbl> 27, 41, 48, 53, 56, 59, 63, 65, 68, 70, 72, 74, 75,...
    ## $ Q94.5      <dbl> 35.055, 52.000, 61.000, 67.000, 72.055, 77.000, 81....
    ## $ Q98.5      <dbl> 47.000, 68.000, 80.000, 88.000, 95.000, 103.000, 10...
