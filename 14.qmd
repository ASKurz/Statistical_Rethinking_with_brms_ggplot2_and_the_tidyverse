# Missing Data and Other Opportunities {#sec-Missing-Data-and-Other-Opportunities}
...
## Measurement error {#sec-Measurement-error}

Let's grab those `WaffleDivorce` data from back in @sec-Spurious-associations.
...
In previous editions of this book, we relied on the dark theme functions from the [**ggdark** package](https://CRAN.R-project.org/package=ggdark) [@R-ggdark] for the plot themes. Though **ggdark** was formerly available on CRAN, it was removed on 2025-11-07 due to some unresolved issues (see [here](https://cran.r-project.org/web/packages/ggdark/index.html)). There's currently a promising looking [pull request](https://github.com/nsgrantham/ggdark/pull/13) on GitHub that may fix the issue, but until the issue is resolved we'll make substitutes by hand.
...
```{r}
b14.1_se$fit@inits |> str()
b14.1_mi$fit@inits |> str()
```
...
# red line
f_error <- fitted(b14.1_mi, newdata = nd) %>%
  data.frame() %>%
  bind_cols(nd)

# yellow line
f_no_error <- fitted(b14.1b, newdata = nd) %>%
  data.frame() %>%
  bind_cols(nd)
...
\begin{align*}
\text{Divorce}_{\text{estimated}, i} & \sim \operatorname{Normal}(\mu_i, \sigma) \\
\mu     & = \alpha + \beta_1 \text A_i + \beta_2 \color{blue}{\text{Marriage\_rate}_{\text{estimated}, i}} \\
\text{Divorce}_{\text{observed}, i}  & \sim \operatorname{Normal}(\text{Divorce}_{\text{estimated}, i}, \text{Divorce}_{\text{standard error}, i}) \\

\color{blue}{\text{Marriage\_rate}_{\text{observed}, i}} & \color{blue}\sim \color{blue}{\operatorname{Normal}(\text{Marriage\_rate}_{\text{estimated}, i}, \text{Marriage\_rate}_{\text{standard error}, i})} \\
...
```{r, warning = F}
nd <- tibble(
  neocortex = seq(from = .5, to = .85, length.out = 30),
  logmass   = median(data_list$logmass))

f_b14.3 <- fitted(b14.3, newdata = nd) %>%
  data.frame() %>%
  bind_cols(nd)
...
```{r}
f_b14.3_mi <- summarise_draws(b14.3, mean, ~quantile2(.x, probs = c(.025, .975))) %>% 
...
p1 <- f_b14.3 %>% 
  ...
  geom_point(data = data_list %>% data.frame(),
             aes(y = kcal),
             color = "white") +
...
```{r}
f_b14.4 <- fitted(b14.4, newdata = nd) %>%
  data.frame() %>%
  bind_cols(nd)

f_b14.4_mi <- summarise_draws(b14.4, mean, ~quantile2(.x, probs = c(.025, .975))) %>% 
  filter(str_detect(variable, "Ymi")) %>%
  bind_cols(data_list %>%
              data.frame() %>%
              filter(is.na(neocortex)))
...
b_model <- bf(div_obs | mi(div_sd) ~ 0 + Intercept + mi(mar_obs) + A) +
...
```{r, eval = F, echo = F}
summarise_draws(b14.2_mi_mi, mean, sd, ~quantile2(.x, probs = c(.025, .975)))
summarise_draws(b14.2_mi, mean, sd, ~quantile2(.x, probs = c(.025, .975)))
```
...
## ~~Summary~~ Bonus: Meta-analysis {#sec-Meta-analysis}
...
In other words, some studies have multiple outcomes. In order to better accommodate the `study`- and `outcome`-level variances, let's fit a cross-classified Bayesian meta-analysis reminiscent of the cross-classified chimp model from @sec-Cross-classified-chimpanzees-with.
