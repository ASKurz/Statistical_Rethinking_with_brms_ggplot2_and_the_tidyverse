...
For more technical details, see the paper. In short, `Bulk_ESS` in the output from brms 2.10.0+ is what was previously referred to as `Eff.Sample` in earlier versions. It's also what corresponds to what McElreath calls `n_eff`. This indexed the number of effective samples in 'the centre of the' posterior distribution (i.e., the posterior mean or median). But since we also care about uncertainty in our parameters, we also care about stability in the 95% intervals and such. The new `Tail_ESS` in brms output allows us to gauge the effective sample size for those intervals.
...
For more ideas on customizing a GGally pairs plot, go [here](https://ggobi.github.io/ggally/#columns_and_mapping).
...
```{r, echo = F}
rm(i, num_weeks, positions, current, proposal, prob_move, d, dd, b8.1, b8.1_4chains_4cores, post, my_diag, my_lower, l_b8.1, b8.2, b8.3, n, y, b8.4, b8.5, p1, p2)
```
