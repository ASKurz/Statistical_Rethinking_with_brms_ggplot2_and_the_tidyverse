...
But if you do want to learn more about HMC, McElreath has some nice introductory lectures on the topic (see [here](https://youtu.be/v-j0UmWf3Us) and [here](https://youtu.be/BWEtS3HuU5A)). To dive even deeper, [Michael Betancourt]( https://twitter.com/betanalpha) from the Stan team has given many lectures on the topic (e.g., [here](https://youtu.be/_fnDz2Bz3h8) and [here](https://youtu.be/jUSZboSq1zg)).
...
```{r, fig.height = 4.5, fig.width = 7.25, warning = F}
# left
p1 <-
  post %>%
  select(b_Intercept) %>%
...
```{r, echo = F}
rm(i, num_weeks, positions, current, proposal, prob_move, d, dd, b8.1, b8.1b, post, my_upper, my_diag, my_lower, l_b8.1, b8.2, b8.3, n, y, b8.4, b8.5, p1, p2)
```
