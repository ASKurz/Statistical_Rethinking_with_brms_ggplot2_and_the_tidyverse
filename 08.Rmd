...
```{r b8.1, echo = F}
# save(b8.1, file = "fits/b08.01.rda")
# rm(b8.1)
load("fits/b08.01.rda")
```
...
Do note a couple things: If you look closely at the summary information at the top, you'll see that the `brm()` function defaults to `chains = 4`. If you check the manual, you'll see it also defaults to `cores = 1`. You'll also note it defaults to `iter = 2000, warmup = 1000`. Also of note, McElreath's `rethinking::precis()` returns highest posterior density intervals (HPDIs) when summarizing `map2stan()` models. Not so with brms. If you want HPDIs, you'll have to use the convenience functions from the tidybayes package. Here's an example.
...
```{r b8.1b, echo = F}
# save(b8.1b, file = "fits/b08.01b.rda")
# rm(b8.1b)
load("fits/b08.01b.rda")
```
...
With this in hand, we can now include those warmup draws in our trace plots. Here's how to do so withougt convenience functions like `bayesplot::mcmc_trace()`.
...
```{r b8.2, echo = F}
# save(b8.2, file = "fits/b08.02.rda")
# rm(b8.2)
load("fits/b08.02.rda")
```
...
```{r b8.3, echo = F}
# save(b8.3, file = "fits/b08.03.rda")
# rm(b8.3)
load("fits/b08.03.rda")
```
...
p1 + p2 + plot_annotation(title = "Prior (dashed) and posterior (solid) distributions for the model with\nweakly-informative priors, b8.3")
```  
...
```{r b8.4, echo = F}
# save(b8.4, file = "fits/b08.04.rda")
# rm(b8.4)
load("fits/b08.04.rda")
```
...
```{r b8.5, echo = F}
# save(b8.5, file = "fits/b08.05.rda")
# rm(b8.5)
load("fits/b08.05.rda")
```
...
```{r, fig.width = 7, fig.height = 5.75, warning = F, message = F}
