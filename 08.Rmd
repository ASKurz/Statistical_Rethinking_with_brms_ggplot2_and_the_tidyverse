...
> where $p_t (\tilde{y}_i)$ is the distributino representing the true data-generating process for $\tilde{y}_i$. The $p_t (\tilde{y}_i)$'s are unknown, and we will use cross-validation or WAIC to approximate. In a regression, these distriubtions are also implicitly conditioned on any predictors in the model. (Vehtari, Gelman, & Gabry, 2016, p. 2).

Later in the paper, we learn the `elpd_loo` (i.e., the Bayesian LOO estimate of out-of-dampel predictive fit) is defined as
