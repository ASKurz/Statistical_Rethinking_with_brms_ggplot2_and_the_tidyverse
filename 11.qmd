# Monsters and Mixtures {#sec-Monsters-and-Mixtures}
...
Recall we use the base-R `plogis()` function in place of McElreath's `logistic()` function to get these into the probability metric.
...
This yielded a posterior mean of 4.2, much like our `average_outcome_value`, above. However, the lazy Gaussian model now has a $\sigma$ parameter, whereas $\sigma$ is a function of the probability estimate in the more-appropriate `b11.2` model (see @sec-Over-dispersed-outcomes). If you only care about mean estimates, this won't matter much if you have a lot of data and the mean is far from the boundary. But when your posterior means get close to the upper or lower boundary, the lazy Gaussian model will yield silly estimates for the 95% intervals. Beware the lazy Gaussian model with data like this.
...
We don't have a `coeftab()` function in brms like for rethinking. But as we did in @sec-Comparing-estimates, we can reproduce it with help from the `posterior::summarise_draws()` function and a bit of data wrangling.
...
```{r}
b11.1 <- add_criterion(b11.1, criterion = "waic")
b11.2 <- add_criterion(b11.2, criterion = "waic")
b11.3 <- add_criterion(b11.3, criterion = "waic")
```
...
## Zero-inflated outcomes {#sec-Zero-inflated-outcomes}
...
>
> \begin{align*}
> \Pr(y \mid p, \lambda) & = \Pr(\text{drink} \mid p) (0) + \Pr(\text{work} \mid p) \Pr(y \mid \lambda) \\
...
## Over-dispersed outcomes {#sec-Over-dispersed-outcomes}
...
If you'd like to see how to make a similar plot in terms of $\alpha$ and $\beta$, see [Chapter 6](https://solomon.quarto.pub/dbda2/06.html) of my [-@kurzDoingBayesianDataAnalysis2026] ebook [translation of Kruschke's text into tidyverse and brms code](https://solomon.quarto.pub/dbda2/).
...
Here's what the corresponding `as_draws_df()` data object looks like.
...
```{r, warning = F}
post %>% 
  tidybayes::median_qi(plogis(b_Intercept)) %>% 
  mutate_if(is.double, round, digits = 3)
```

To stay within the tidyverse while making the many thin lines in Figure 11.5.a, we're going to need to do a bit of data processing. First, we'll want a variable to index the rows in `post` (i.e., to index the posterior draws). And we'll want to convert the `b_Intercept` to the $\bar p$ metric with the `plogis()` function. Then we'll use `slice_sample()` to randomly draw a subset of the posterior draws. Then with the `expand_grid()` function, we'll insert a tightly-spaced sequence of `x` values ranging between 0 and 1--the parameter space of beta distribution. Finally, we'll use the `rethinking::dbeta2()` function to compute the density values for the unique combination of `x`, `p_bar`, and `phi` values in each row.

```{r}
set.seed(11)

lines <- post %>% 
  mutate(p_bar = plogis(b_Intercept)) %>% 
  slice_sample(n = 100) %>% 
...
lines %>% 
  ...
  stat_function(fun = rethinking::dbeta2,
                args = list(prob = mean(plogis(post$b_Intercept)),
                            theta = mean(post$phi)),
...
# Quarto is snagging on this step for some unclear reason
# set.seed(11)
# p_b11.5 <- predict(b11.5)
# save(p_b11.5, file = "sims/p_b11.5.rda")

load("sims/p_b11.5.rda")
```
...
predict(b11.5) %>%
  ...
  bind_cols(fitted(b11.5) %>% data.frame()) %>% 
...
```{r, fig.width = 4, fig.height = 3.5, echo = F}
# this line is the only change from the block above
p_b11.5 %>%
  data.frame() %>% 
  transmute(ll = Q2.5,
            ul = Q97.5) %>%
...
### Negative-binomial or gamma-Poisson {#sec-Negative-binomial-or-gamma-Poisson}
...
```{r, fig.width = 4.5, fig.height = 4.5, warning = F}
# data
crossing(mu    = c(1, 5, 9),
         theta = c(1, 5, 9)) %>% 
...
b11.6 <- brm(
  ...
  admit ~ 1,
  prior = c(prior(normal(0, 10), class = Intercept),
            prior(inv_gamma(0.4, 0.3), class = shape)),  # this is the brms default
  iter = 4000, warmup = 1000, cores = 2, chains = 2,
  seed = 11,
  file = "fits/b11.06")
```

You may have noticed we used the brms default `prior(inv_gamma(0.4, 0.3), class = shape)` for the shape parameter. Sadly, the base-R stats package does not have convince functions for the inverse gamma distribution, but we do have access to a `dinvgamma()` function from the invgamma package. Here's what that prior looks like.
...
$$\operatorname{Gamma}(y \mid \mu, \alpha) = \frac{\left (\frac{\alpha}{\mu} \right )^\alpha}{\Gamma (\alpha)} y^{\alpha - 1} \exp \left (- \frac{\alpha y}{\mu} \right ),$$
...
where $\alpha$, $e$, and $\Gamma$ are all as they were before and $\beta$ is the rate parameter. Let's say we wanted to use the $\operatorname{Gamma}(y \mid \alpha, \theta)$ parameterization with the `dgamma()` function. Here are the posterior means for our $\alpha$ and $\theta$ parameters.
...
If you'd like more practice with the negative-binomial model and some of the others models for categorical data we covered in this chapter and in @sec-Counting-and-Classification, Alan Agresti covered them extensively in his [-@agrestiFoundationsLinearGeneralized2015] text, [*Foundations of linear and generalized linear models*](https://www.wiley.com/en-us/Foundations+of+Linear+and+Generalized+Linear+Models-p-9781118730034).
