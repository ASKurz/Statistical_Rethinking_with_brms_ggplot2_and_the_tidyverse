# Counting and Classification {#sec-Counting-and-Classification}
...
The base-R `plogis()` function will be our alternative to the `logistic()` function in rethinking.^[brms also has a `inv_logit_scaled()` function, which is a generalization of `plogis()`. For our purposes, though, `plogis()` is just fine.] Here we use it to convert the 89% interval estimates McElreath reported on page 294.
...
ppa <- pp_average(b10.1, b10.2, b10.3,
                  weights = "waic",
                  method = "fitted") %>%
  data.frame() %>% 
...
\begin{align*}
\text{pulled\_left}_i      & \sim \operatorname{Binomial}(1, p_i) \\
\operatorname{logit}(p_i) & = \alpha_{\text{actor}} + (\beta_1 + \beta_2 \text{condition}_i) \text{prosoc\_left}_i \\
...
```{r}
b10.4 <- add_criterion(b10.4, criterion = "waic")
...
```{r, message = F}
b10.6 <- add_criterion(b10.6, criterion = "waic")
b10.7 <- add_criterion(b10.7, criterion = "waic")
```
...
```{r}
b10.6 <- add_criterion(b10.6, criterion = "loo")
b10.7 <- add_criterion(b10.7, criterion = "loo")
```
...
post %>%
  mutate(p_admit_male   = plogis(b_Intercept + b_male),
         p_admit_female = plogis(b_Intercept)) %>% 
...
### Example: Oceanic tool complexity {#sec-Example-Oceanic-tool-complexity}
...
\log (\lambda_i)     & = \alpha + \beta_1 \text{log\_pop}_i + \beta_2 \text{contact\_high}_i + \beta_3 \text{contact\_high}_i \times \text{log\_pop}_i \\
...
```{r, fig.width = 3, fig.height = 2.9}
# intermediary tibbles for our the dot and line portion of the plot
point_tibble <- tibble(
  x = c(median(post$b_contact_high), min(post$b_contact_high)),
  y = c(min(post$`b_log_pop:contact_high`), median(post$`b_log_pop:contact_high`)))

line_tibble <- tibble(
  parameter = rep(c("b_contact_high", "b_log_pop:contact_high"), each = 2),
...
# no contact rate
b10.12 <- update(
  b10.10, formula = total_tools ~ 1 + log_pop,
  seed = 10,
  file = "fits/b10.12")

# no log-population
b10.13 <- update(
  b10.10, formula = total_tools ~ 1 + contact_high,
  seed = 10,
  file = "fits/b10.13")

# intercept only
b10.14 <- update(
  b10.10, formula = total_tools ~ 1,
  seed = 10,
  file = "fits/b10.14") 
```
...
We've been using `bayesplot::mcmc_pairs()` a lot for our posterior pairs plots. Let's get in a little practice with `GGally::ggpairs()` for Figure 10.10. In @sec-Multicollinear-milk and @sec-Visualization, we used custom functions to augment the default `ggpairs()` output. We'll continue that trend, here. Here are the custom functions for the upper triangle, the diagonal, and the lower triangle.
...
The next two of the remaining four models are maximum entropy distributions for certain problem types. The last two are mixtures, of which we'll see more in @sec-Zero-inflated-outcomes.

### Multinomial {#sec-Multinomial}

> When more than two types of unordered events are possible, and the probability of each type of event is constant across trials, then the maximum entropy distribution is the multinomial distribution. [We] already met the multinomial, implicitly, in @sec-Big-Entropy-and-the-GLM when we tossed pebbles into buckets as an introduction to maximum entropy. The binomial is really a special case of this distribution. And so its distribution formula resembles the binomial, just extrapolated out to three or more types of events. If there are $K$ types of events with probabilities $p_1, \dots, p_K$, then the probability of observing $y_1, \dots, y_K$ events of each type out of $n$ trials is (p. 323):

$$\Pr (y_1, ..., y_K \mid n, p_1, ..., p_K) = \frac{n!}{\prod_i y_i!} \prod_{i = 1}^K p_i^{y_i}$$

Compare that equation with the simpler version in section 2.3.1 (page 33 in the text).

#### Explicit multinomial models {#sec-Explicit-multinomial-models}
...
as_draws_df(b10.16_true) %>% 
  transmute(b = plogis(b_b_Intercept)) %>% 
  mean_qi(b) %>% 
  mutate_if(is.double, round, digits = 2)
```
...
$$\Pr(k \mid s_1, s_2, ..., s_K) = \frac{\exp (s_k)}{\sum_{i = 1}^K \exp (s_i)},$$
...
as_draws_df(b10.16_true) %>% 
  transmute(draw = .draw,
            s1   = plogis(b_b_Intercept) * income[1],
            s2   = plogis(b_b_Intercept) * income[2],
            s3   = plogis(b_b_Intercept) * income[3]) %>% 
