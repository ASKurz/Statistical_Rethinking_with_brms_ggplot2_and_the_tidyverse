...
Now we'll get both predictors in there with our very first multivariable model. If we let $x_1$ be `Marriage_s` and $x_2$ be `MedianAgeMarriage_s`, we can write the statistical model as
...
Have you ever seen a $\rho = .9995$ correlation, before? Here it is in a plot.
...
That posterior $SD$ looks much better. Here's the density in Figure 5.8.b.
...
$$\mu_i = \alpha_{female} (1 - male_i) + \alpha_{male} male_i$$

So then a `female` dummy would satisfy the condition $female_i = (1 - male_i)$. Let's make that dummy.
...
```{r, echo = F}
rm(d, b5.1, nd, fitd_b5.1, b5.2, fitd_b5.2, b5.3, post, b5.4, fitd_b5.4, res_b5.4, text, b5.4b, N, b5.5, dcc, b5.6, b5.7, rho, b5.O.both, b5.O.pos, b5.O.neg, b5.8, b5.9, b5.10, b5.11, b5.12, my_diag, my_lower, sim_coll, n_seed, n_rho, b5.13, b5.14, b5.15, b5.15b, b5.16, b5.16_alt)
```
