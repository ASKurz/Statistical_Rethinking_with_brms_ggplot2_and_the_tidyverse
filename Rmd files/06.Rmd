...
McElreath's not a fan, but we will cover the Bayesian version of $R^2$ and how to use it in brms at the end of the chapter.
...
Okay, now we're ready to stitch the six subplots together and produce our version of Figure 6.3.
...
Zero. Our intercept-only `b6.7` explained exactly zero variance in `brain`. All it did was tell us what the unconditional mean and variance (i.e., 'Residual standard error') were. I hope that makes sense. They were the only things in the model: $v_i \sim \text{Normal}(\mu = \alpha, \sigma)$. To get the intercept-only model for Figure 6.4, we plug `formula = y ~ 1` into the `stat_smooth()` function.
...
Here's how the newcomer faired:
...
we can compute $D_{\text{KL}} (p, q)$ within a tibble like so:
...
Our systems in this section are binary (e.g., $q = \lbrace q_i, q_2 \rbrace$). Thus if you know $q_1 = .3$ you know of a necessity know $q_2 = 1 - q_1$. Therefore we can write the tibble for the next example of when $p = q$ like this:
...
```{r sim_1, cache = T, warning = F, message = F, results = 'hide'}
library(rethinking)
...
```{r sim_2, cache = T, warning = F, message = F}
library(rethinking)
...
In the text, McElreach focuses on the DIC and WAIC. As you'll see, the LOO has increased in popularity since he published the text. Going forward, we’ll juggle the WAIC and the LOO in this project. But we will respect the text and work in a little DIC talk.
...
```{r w_b6.11_throughw_b6.14, cache = T, message = F, warning = F}
w_b6.11 <- waic(b6.11)
w_b6.12 <- waic(b6.12)
w_b6.13 <- waic(b6.13)
w_b6.14 <- waic(b6.14)
...
Our new `compare_waic()` returns the following:
...
Now we'll use `waic_tibble()` and `bind_rows()` to make a multi-model tibble suitable for plotting the WAIC estimates.
...
```{r, echo = F}
rm(d, fit_lm, fits, p, p1, p2, p3, p4, p5, p6, b6.7, nd, make_lines, lines, weatherperson, newcomer, t, inits, inits_list, b6.8, dfLL, N, kseq, n_sim, n_cores, dev_20, dev_100, dev_tibble, dev_text, make_sim, s, b, dfmean, lppd, dfvar, pwaic, b6.11, b6.12, b6.13, b6.14, w_b6.11, w_b6.12, w_b6.13, w_b6.14, compare_waic, waic_tibble, w_d, diff, my_coef_tab, p11, p12, p13, p14, fitd_b6.14, r2_b6.13, r2_b6.14, r2_combined)
```
