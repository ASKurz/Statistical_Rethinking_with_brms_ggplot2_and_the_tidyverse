...
samples <- tibble(big   = map_dbl(1:10000, ~ prod(1 + runif(12, 0, 0.5))),
...
## A Gaussian model of height {#sec-A-Gaussian-model-of-height}
...
This is the point in the project where we hop off the grid-approximation train. On the one hand, I think this is a great idea. Most of y'all reading this will never use grid approximation in a real-world applied data analysis. On the other hand, there is some pedagogical utility in practicing with it. It can help you grasp what it is we're dong when we apply Bayes' theorem. If you'd like more practice, check out the first several chapters in John Kruschke's [-@kruschkeDoingBayesianData2015] [textbook](https://sites.google.com/site/doingbayesiandataanalysis/) and the corresponding chapters in my [-@kurzDoingBayesianDataAnalysis2026] [ebook](https://solomon.quarto.pub/dbda2/) translating it into **brms** and **tidyverse**.
...
After running model fit with HMC, it's a good idea to inspect the chains. As we'll see, McElreath covered this in @sec-Checking-the-chain. Here's a typical way to do so with brms.
...
f <- fitted(b4.5, newdata = weight_seq) %>%
  data.frame() %>%
  bind_cols(weight_seq)

p <- predict(b4.5, newdata = weight_seq) %>%
  data.frame() %>%
  bind_cols(weight_seq)  
```
...
```{r, warning = F, fig.width = 3, fig.height = 3}
f <- fitted(b4.6, newdata = weight_seq) %>%
  data.frame() %>%
  bind_cols(weight_seq)

p <- predict(b4.6, newdata = weight_seq) %>%
  data.frame() %>%
  bind_cols(weight_seq) 
...
```{r, warning = F, fig.width = 3, fig.height = 3}
f <- fitted(b4.7, newdata = weight_seq) %>%
  data.frame() %>%
  bind_cols(weight_seq)

p <- predict(b4.7, newdata = weight_seq) %>%
  data.frame() %>%
  bind_cols(weight_seq) 
