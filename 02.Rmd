...
There’s a lot going on in that output, which we’ll start to clarify in Chapter 4. For now, focus on the 'Intercept' line. As we'll learn about in Chapter 10, the brms default is to use a link function for binomial regression. In this case, that function was the logit, which is indicated in the second line of the output,  `Links: mu = logit`. That also means that the summary is not in the probability metric. But no worries. We can transform the results back into a proper probability metric with the `brms::inv_logit_scaled()` function. We'll then plot the results to compare them with those from `rethinking::map()`, above.
...
If you’re still confused. Cool. This is just a preview. We’ll start walking through fitting models in brms in Chapter 4.
