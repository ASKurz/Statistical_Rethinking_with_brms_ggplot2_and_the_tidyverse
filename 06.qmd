# Overfitting, Regularization, and Information Criteria {#sec-Overfitting}
...
McElreath spent little time discussing $p$-values and null hypothesis testing in the text. If you'd like to learn more from a Bayesian perspective, you might check out the first several chapters (particularly 10--13) in Kruschke's [-@kruschkeDoingBayesianData2015] [text](https://sites.google.com/site/doingbayesiandataanalysis/) and my [-@kurzDoingBayesianDataAnalysis2026] [ebook translating it to brms and the tidyverse](https://solomon.quarto.pub/dbda2/). The great [Frank Harrell](https://www.fharrell.com/) has complied [*A litany of problems With p-values*](https://www.fharrell.com/post/pval-litany/) and you might consult the recent statement on $p$-values by the American Statistical Association [@wassersteinASAStatementonPValues2016].
...
McElreath is not a fan of the $R^2$. But it's important in my field, so instead of a chapter summary in @sec-R2-talk, we will cover the Bayesian version of $R^2$ and how to use it in brms.
...
make_lines <- function(row) {
  ...
  nd %>% 
    mutate(brain =  predict(my_fit, nd))
}
```
...
b6.8 <- brm(
  ...
  init = inits_list,  # here we insert our start values
  seed = 6,
  file = "fits/b06.08")
```
...
Anyway, the brms `log_lik()` function returns a matrix. Each occasion gets a column and each HMC chain iteration gets a row. To make it easier to understand the output, we'll name the columns by `species` using the `set_names()` function.

```{r}
ll <- b6.8 %>%
  log_lik() %>%
  data.frame() %>% 
  set_names(d %>% pull(species))
...
dev_tibble <- rbind(dev_20, dev_100) %>% 
  ...
  mutate(n     = factor(n, levels = c("n = 20", "n = 100")),
         n_par = str_remove(n_par, "X") %>% as.double()) %>% 
...
s <- tibble(n       = rep(c(20, 100), each = 3),
...
s <- tibble(b_sigma = c(100, .5)) %>% 
  mutate(sim = purrr::map(b_sigma, make_sim)) %>% 
  unnest(sim)
```

```{r, echo = F}
# That simulation takes 3.682589 hours to complete. Instead of completing it on the fly 
# for each update, I've saved the results in an external file. 
...
#### Comparing WAIC values {#sec-Comparing-WAIC-values}
...
```{r}
b6.11 <- add_criterion(b6.11, criterion = "waic")
```
...
```{r w_b6.12_through_b6.14, warning = F}
# compute and save the WAIC information for the next three models
b6.12 <- add_criterion(b6.12, criterion = "waic")
b6.13 <- add_criterion(b6.13, criterion = "waic")
b6.14 <- add_criterion(b6.14, criterion = "waic")
...
#### Comparing estimates {#sec-Comparing-estimates}
...
```{r}
my_coef_tab <- tibble(model = c("b6.11", "b6.12", "b6.13", "b6.14")) %>% 
...
I'm also not aware of an efficient way in brms to reproduce Figure 6.12 for which McElreath nested his `coeftab()` argument in a `plot()` argument. However, if you’re willing to deviate just a bit from the format of McElreath's `coeftab()` plot, here's an elegant way to work with our `my_coef_tab` tibble.
...
## ~~Summary~~ Bonus: $R^2$ talk {#sec-R2-talk}

At the beginning of the chapter (pp. 167--168), McElreath briefly introduced $R^2$ as a popular way to assess the variance explained in a model. He pooh-poohed it because of its tendency to overfit. It's also limited in that it doesn't generalize well outside of the single-level Gaussian framework, which will be a big deal for us starting in @sec-Counting-and-Classification. However, if you should find yourself in a situation where $R^2$ suits your purposes, the brms `bayes_R2()` function might be of use. Simply feeding a model `brm()` fit object into `bayes_R2()` will return the posterior mean, $SD$, and 95% intervals. For example:
...
# model `b6.14`
r2_b6.14 <- bayes_R2(b6.14, summary = F) %>%
  data.frame() %>%
  rename(r2_14 = R2)

# let's put them in the same data object
r2_combined <- bind_cols(r2_b6.13, r2_b6.14) %>%
